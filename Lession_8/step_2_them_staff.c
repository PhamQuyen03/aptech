 ...
 *
 * Nội dung mới của giai đoạn:
 * - Thêm struct Sach và struct PhieuMuon, quản lý bằng 2 mảng động mới
 * - Menu Staff: thêm/xem/cập nhật/xóa/tìm kiếm/sắp xếp sách
 * - Nghiệp vụ mượn sách, trả sách, danh sách sách đang được mượn
 */
...
void tra_sach()
{
    ...
    printf("Trả sách thành công! Đã ghi nhận \"%s\" trả sách mã %s.\n",
           ds_nguoi_dung[tim_nguoi_dung_theo_id(ds_nguoi_dung, so_luong_nguoi_dung, id_sv)].ten_dang_nhap, ma);
}
